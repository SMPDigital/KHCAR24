...
function zoomText(){
    ...
    if(accessibilityPropeties.zoomLevel > 0){
        ...
        tooltipElem.attr('tooltip', `Bigger Text ${accessibilityPropeties.zoomLevel}x`);
    }
}
...
function openReadingMask(update){
    if(update){
        accessibilityPropeties.readingMaskActive = !accessibilityPropeties.readingMaskActive;
        updatePropeties(true);
...
function highlightLinks(update){
    ...
    if(update){
        accessibilityPropeties.highlightLinkActive = !accessibilityPropeties.highlightLinkActive;
        updatePropeties(true);
    }
...
function invertColors(update){
    ...
    if(update){
        accessibilityPropeties.invertColorsActive = !accessibilityPropeties.invertColorsActive;
        updatePropeties(true);
    }
...
function updatePropeties(update){
    ...
    if(!localSettings || update){
        localStorage.setItem("accessibilityPropeties", JSON.stringify(accessibilityPropeties));
        markActivated();
        return;
    }
...
function markActivated(){
    $('#zoom_text, #reading_mask, #highlight_link, #invert_colors').removeClass('activated');
